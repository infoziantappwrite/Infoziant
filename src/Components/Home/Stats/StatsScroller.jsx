...
const stats = [
  ...
  {
    label: "Companies Hiring per College",
    value: 115,
    suffix: "+",
    icon: Briefcase,
  ...
  {
    label: "Industry Collaborations & CoEs",
    value: 53,
    suffix: "+",
    icon: Building2,
  ...
  const renderStatCard = (stat, index, type = "row1") => {
    ...
    const widthClass =
      type === "row1"
        ...
        : "w-full sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-0.75rem)]";
    ...
    return (
      ...
      >
        <div className="relative p-3 sm:p-5 flex flex-col items-center text-center h-full">
          ...
          <div className="mt-1">
            <div
              className={`text-xl sm:text-3xl font-bold mb-1 bg-gradient-to-br ${stat.color} bg-clip-text text-transparent`}
            >
              {startCount && (
                <CountUp
                  start={0}
    ...
    >
      <div className="flex flex-wrap justify-center gap-3 max-w-screen-xl mx-auto">
        ...
        {stats.slice(0, 4).map((stat, index) =>
          renderStatCard(stat, index, "row1")
        )}
      </div>

      <div className="flex flex-wrap justify-center gap-3 mt-8 max-w-screen-xl mx-auto">
        {/* Second row: 4 to 6 */}
        {stats.slice(4).map((stat, index) =>
          renderStatCard(stat, index + 4, "row2")
        )}
      </div>
    </section>
  );
