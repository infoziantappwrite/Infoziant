...
export default function StatsScroller() {
  ...
  return (
    <motion.section
      ref={ref}
      className="relative bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 py-20 px-4 sm:px-6 md:px-10 xl:px-16 overflow-hidden"
      initial={{ opacity: 0, y: 40 }}
      animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 40 }}
      transition={{ duration: 0.8, ease: "easeOut" }}
    >
      {/* Blurred circles and gradient layer */}
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-white to-transparent opacity-[0.03]"></div>

      <div className="relative z-10 flex flex-wrap justify-center gap-3 max-w-screen-xl mx-auto">
        {stats.map((stat, index) => (
          <motion.div
            key={index}
            className="group relative w-full sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-0.75rem)] xl:w-[calc(25%-0.75rem)] overflow-hidden rounded-xl bg-gray-900 text-white shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer"
            initial={{ opacity: 0, y: 40 }}
          ...
          >
            <div className="relative p-5 flex flex-col items-center text-center h-full">
              <div className={`mb-3 w-14 h-14 flex items-center justify-center rounded-full bg-gradient-to-br ${stat.color} shadow-md group-hover:shadow-lg transform transition-transform duration-300 group-hover:scale-110`}>
                <stat.icon size={22} className="text-white" strokeWidth={2} />
              </div>
              <div className="mt-1">
              ...
              <motion.div
                className={`absolute bottom-0 left-0 right-0 h-[3px] bg-gradient-to-r ${stat.color}`}
                initial={{ scaleX: 0 }}
                whileHover={{ scaleX: 1 }}
