          ...
          <Slider ref={sliderRef} {...settings} className="client-slider px-6 mx-auto">
            {clients.map((client, index) => (
              <div key={client.id} className="px-2 py-6 flex justify-center items-center">
                <div
                  className={`relative overflow-hidden rounded-xl transition-all duration-700 mx-auto ${index === centerIndex
                    ? ' transform scale-105'
                    : 'bg-blue-900/20 transform scale-95'
                    }`}
                >
                  {/* Client Title (outside of image card) */}
                  ...
                  {/* Image Card */}
                  <div
                    className={`flex flex-col items-center transition-all duration-700 ${index === centerIndex ? 'scale-100 opacity-100' : 'scale-90 opacity-70'
                      }`}
                  >
                    <div className="h-full w-full relative overflow-hidden rounded-xl bg-gradient-to-br from-blue-800/50 to-teal-700/30 p-[3px]">
                      <div className="w-full h-full overflow-hidden flex items-center justify-center rounded-lg px-5 py-3">
                        <img
                          src={client.image}
                          alt={client.title}
